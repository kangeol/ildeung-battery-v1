...
export const batteryKnowledgeCopy = Object.freeze({
 ...
 cold:'추운 환경에서는 배터리의 시동 성능이 약해질 수 있습니다. 추위만으로 고장을 단정할 수는 없으니 시동 상태와 충전 상태를 함께 확인해 주세요.',
...
export function batteryKnowledgePlan(text,state){
 const s=String(text).normalize('NFKC').replace(/\s/g,'').toLowerCase().replace(/^혹시/,''),topic=state.lastIntent||'',keys=[];
 ...
 const cca=/cca(?![a-z])|씨씨에이/.test(s)||topic==='KNOWLEDGE_CCA'&&/^(?:높으면|제차는몇|내차는몇|몇이어야)[?？.!]*$/.test(s);
 ...
 if(/^(?:추우면|겨울|겨울에약해지나요)[?？.!]*$|(?:배터리|겨울|추위).*(?:추우|겨울|약해)/.test(s))keys.push('cold');
 if(/점프.*(?:그냥|써도|괜찮)/.test(s))keys.push('jump');
 if(!keys.length&&!fit)return null;
 return {keys:[...new Set(keys)],fit,fee,topic:coding?'CODING':cca?'CCA':capacity||fit?'CAPACITY':discharge?'DISCHARGE':'CONDITION'};
}
