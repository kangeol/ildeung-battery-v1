...
const width=Number(process.env.UAT_WIDTH||1440),session='consult-ui-'+width;
...
const save=()=>{fs.mkdirSync('docs/evidence/consult-ui',{recursive:true});fs.writeFileSync(`docs/evidence/consult-ui/browser-${width}.json`,JSON.stringify(evidence,null,2)+'\n');};
...
function ask(input,family){const before=snapshot();run('fill','#chatInput',input);run('click','#sendButton');const after=snapshot(),want=conversationTurn(before.state,input,rows,areas,catalog,policy),messages=after.messages.slice(before.messages.length).filter(m=>m.role==='bot').map(m=>m.text);assert.deepEqual(after.state,want.state);assert.deepEqual(messages,want.messages);if(family)for(const re of expected(family,input))assert.match(messages.join(' '),re);const displayed=ev("Array.from(document.querySelectorAll('.answer-row')).at(-1)?Array.from(Array.from(document.querySelectorAll('.answer-row')).at(-1).querySelectorAll('.answer-text')).map(p=>p.textContent):[]");assert.deepEqual(displayed,want.messages,'verbatim DOM answer');evidence.push({width,family,input,messages,state:after.state});return after;}
...
const questions=['현금되나요?','정품인가요?','예약되나요?','오늘 되나요?','몇 월 생산이에요?','델코랑 바르타 차이','델코와 바르타 차이','둘이 뭐가 달라요?','뭐가 더 좋아요?','가격 차이는?','왜 바르타가 더 비싸요?','수명 차이 나요?','성능 차이 있어요?','둘 다 정품이에요?','둘 다 실버인가요?','어디서 만들어요?'];
...
for(const [name,flow]of [['faq',['현금되나요?']],['price',['BMW 5시리즈 2020년식']],['compare',['델코랑 바르타 차이']],['context',['BMW 5시리즈 2020년식','바르타는?','둘 차이가 뭐야?']],['realtime',['오늘 되나요?']],['date',['몇 월 생산이에요?']],['candidates',['G90']]]){reset();for(const q of flow)ask(q);const metrics=ev("Array.from(document.querySelectorAll('.answer-text,.price-amount,.answer-context,.quick-reply:not(:disabled),#chatInput,#sendButton')).map(e=>({class:e.className,id:e.id,width:e.getBoundingClientRect().width,height:e.getBoundingClientRect().height,font:getComputedStyle(e).fontSize,line:getComputedStyle(e).lineHeight,color:getComputedStyle(e).color,bg:getComputedStyle(e).backgroundColor}))");assert.ok(metrics.filter(e=>e.class.includes('quick-reply')).every(e=>e.height>=44&&parseFloat(e.font)>=15));assert.ok(metrics.filter(e=>e.id==='chatInput').every(e=>parseFloat(e.font)>=16&&e.height>=48));assert.equal(ev("document.documentElement.scrollWidth>innerWidth"),false);evidence.push({width,visual:name,metrics,screenshot:run('screenshot')});}
...
evidence.push({width,ui,reentry:true,screenshot:run('screenshot')});
if(width===390){run('set','viewport','390','480');run('wait','#chatInput');run('click','#chatInput');ask('카드');const keyboard=ev("({bodyScroll:scrollY,overflow:document.documentElement.scrollWidth>innerWidth,inputBottom:document.querySelector('#chatInput').getBoundingClientRect().bottom,height:innerHeight})");assert.ok(keyboard.inputBottom<=keyboard.height);assert.equal(keyboard.bodyScroll,0);evidence.push({keyboardViewportEmulation:keyboard});run('set','viewport','390','844');}
run('set','media','light','reduced-motion');reset();ask('AGM105');assert.equal(ev("getComputedStyle(document.querySelector('.answer-enter')).animationName"),'none');evidence.push({width,reducedMotion:true});save();run('close');console.log('PASS '+evidence.length+' records');
