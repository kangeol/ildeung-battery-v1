...
if(process.argv.includes('--browser')){
 ...
 // Authorized Tier2 "달라" routing changes the prose of non-waste questions.
 // Preserve the substantive boundary instead of freezing historical strings:
 // genuine brand comparisons remain brand answers; other differences must not
 // acquire either waste policy or a DELKOR/VARTA clarification.
 const nonWaste=(o,q,brand)=>{const answer=o.messages.join(' ');return !/폐배터리 수거 조건|폐배터리를 보관|미수거/.test(answer)&&(brand?/델코|바르타/.test(answer):!/어떤 제품이나 브랜드를 비교/.test(answer));};
 for(const [i,q]of negatives.entries())for(const s of contexts){const o=turn(q,s,false);check(i<5?names[9]:names[10],nonWaste(o,q,i<5),q);}
 ...
 for(const s of contexts){const kept=turn('폐배터리 안 주면?',s).state;for(const [i,q]of negatives.entries()){const o=turn(q,kept,false);check(names[10],nonWaste(o,q,i<5),q);}}
 ...
 assert.equal(execFileSync('git',['diff',postSyncBaseline,'--','*.html','*.css','data','seo-data','sitemap.xml'],{encoding:'utf8'}),'');
