...
const OUTPUT_DIR = path.join(ROOT_DIR, "assets", "seo", "area");
...
function areaImagePath(...segments) {
  return `/assets/seo/area/${segments.join("/")}.png`;
}
...
function buildThumbnailSpecs(data) {
  const areas = getAreas(data);
  const specs = [{
    imagePath: areaImagePath("index"),
    eyebrow: "NO.1 BATTERY",
    topLine: "서울·경기·인천",
    mainText: "출장배터리"
  }];
  ...
  areas.forEach((area) => {
    specs.push({
      imagePath: areaImagePath(area.slug, "index"),
      eyebrow: "NO.1 BATTERY",
      topLine: "서비스 지역",
      mainText: `${area.name} 출장배터리`
    });

    area.regions.forEach((region) => {
      specs.push({
        imagePath: areaImagePath(area.slug, region.id),
        eyebrow: "NO.1 BATTERY",
        topLine: area.name,
        mainText: `${getRegionMainName(region)} 출장배터리`
      });

      region.neighborhoods.forEach((neighborhood) => {
        specs.push({
          imagePath: areaImagePath(area.slug, region.id, neighborhood.slug),
          eyebrow: "NO.1 BATTERY",
          topLine: `${area.name} ${region.name}`,
          mainText: `${neighborhood.name} 출장배터리`
        });
      });
    });
  });
...
async function generate() {
  console.log("Area Thumbnail Generate Start");
  console.log("");

  const data = readJson(AREA_DATA_FILE);
  const specs = buildThumbnailSpecs(data);
  ...
  console.log("");
  console.log("Area Thumbnail Generate Complete");
}
