...
function renderCapacityPage({ capacity, rows }, vehiclePageMap) {
  ...
  const topic = {
    ...
    directAnswer: `${capacity} 적용 여부는 차량 제조사와 연식, 연료 및 세부모델에 따라 다릅니다. 일등밧데리 차량 DB에서 ${capacity}이 등록된 차량 정보를 기준으로 적용 차량을 확인할 수 있습니다.`,
    faqs: [
      [`${capacity} 배터리 가격은 얼마인가요?`, "가격은 제품과 판매 조건에 따라 달라질 수 있습니다. AGM 배터리 최저가 바로가기에서 현재 판매가격을 확인해 주세요."],
      [`${capacity}가 표시된 차량은 모두 같은 배터리인가요?`, "DB에 같은 용량이 표시되어도 차량별 세부 사양과 장착 조건은 다를 수 있습니다."],
