...
export const postSyncBaseline='c26840e91c3bdad49178fa0bafae77334f108a76';
export const approvedBlogCount=348;
...
const latestBlogFiles=new Set(git('diff','--name-only','c2a9556b8105995852bdec6c29635eea1cada509',postSyncBaseline).toString().trim().split(/\r?\n/));
export const approvedSyncFiles=new Set([...previousSyncFiles,...latestBlogFiles]);
for(const p of approvedSyncFiles){const before=git('show',(latestBlogFiles.has(p)?postSyncBaseline:previousSyncBaseline)+':'+p),now=fs.readFileSync(p);if(p.endsWith('.webp'))assert.deepEqual(now,before,p);else assert.equal(now.toString().replace(/\r\n/g,'\n'),before.toString().replace(/\r\n/g,'\n'),p+' post-sync freeze');}
