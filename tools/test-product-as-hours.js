...
assert.deepEqual(policy.product,{
...
for(const p of changedHtml){let expected=git(['show',`${baseline}:${p}`]).replace(/\r\n/g,'\n');expected=p==='smart-consult/index.html'?expected.replace('?v=brand-v1','?v=product-v1'):expected.replace(/\bDIN70L\b/g,'DIN74L');assert.equal(fs.readFileSync(p,'utf8').replace(/\r\n/g,'\n'),expected);}
...
fs.mkdirSync('docs/evidence/product-as-hours',{recursive:true});fs.writeFileSync('docs/evidence/product-as-hours/product-as-hours.json',JSON.stringify({status:'PASS',metrics,beforeDin70:1,afterDin70:0,changedHtml,transcripts},null,2)+'\n');console.log({status:'PASS',turns:transcripts.length,metrics,changedHtml});
