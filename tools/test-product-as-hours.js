...
const beforePrices=old('data/battery-prices.json');assert.deepEqual(catalog,{...beforePrices,version:catalog.version,unpriced:[],nonAgmBrandPolicy:{brand:'DELKOR',scope:'canonical_non_agm',authority:'Owner: ILDEUNG_AI_CONSULT_OWNER_NON_AGM_DELKOR_BRAND_POLICY_V1'}});assert.equal(catalog.prices.DIN74L,125000);assert.ok(!JSON.stringify(catalog).includes('DIN70L'));
...
for(const input of productInputs)for(const state of [createConversationState(),bmw.state]){
 if(input.includes('브랜드')){const o=turn(state,input);assert.equal(o.state.lastIntent,'BRAND_QUERY');assert.deepEqual({...o.state,lastIntent:state.lastIntent},state);assert.match(o.messages.join(' '),/델코/);assert.match(o.messages.join(' '),/바르타/);continue;}
...
const changedHtml=git(['diff',baseline,'--name-only','--','*.html']).trim().split('\n').filter(p=>p&&p!=='index.html'&&!approvedSyncFiles.has(p));assert.deepEqual(changedHtml,['car-battery/chevrolet/alpheon.html','smart-consult/index.html']);
