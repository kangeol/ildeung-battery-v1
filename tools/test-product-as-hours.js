...
for(const p of changedHtml){let expected=git(['show',`${baseline}:${p}`]).replace(/\r\n/g,'\n');expected=p==='smart-consult/index.html'?expected.replace('?v=brand-v1','?v=location-v1'):expected.replace(/\bDIN70L\b/g,'DIN74L');assert.equal(fs.readFileSync(p,'utf8').replace(/\r\n/g,'\n'),expected);}
...
const evidenceDir=process.argv.includes('--location')?'docs/evidence/location-nlu/regression':'docs/evidence/product-as-hours';
