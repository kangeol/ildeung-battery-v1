...
const transcripts=[];
function assertAuthorizedAmounts(messages){for(const message of messages)for(const match of message.matchAll(/([A-Z0-9-]+) 교체 가격은 ([\d만천]+원)/g)){assert.ok(Object.hasOwn(expected,match[1]),match[1]);assert.equal(match[2],formatWon(expected[match[1]]));}}
...
for(const path of ['data/battery-prices.json','js/smart-consult-prices.js','js/smart-consult-conversation.js','js/smart-consult.js'])assert.ok(!/270000|27만원/.test(fs.readFileSync(path,'utf8')),path);
...
for(const m of manufacturers){const old=JSON.parse(git(['show',`${baseline}:data/${m.file}`]));if(m.file==='hyundai.json')old[40].defaultBattery='AGM60';assert.deepEqual(read(`data/${m.file}`),old);}
const htmlChanges=git(['diff',baseline,'--name-only','--','*.html']).trim().split('\n').filter(Boolean);
assert.deepEqual(htmlChanges.sort(),['car-battery/hyundai/santafe.html','car-battery/hyundai/santafe/tm.html','smart-consult/index.html'].sort());
for(const path of htmlChanges){let expectedHtml=git(['show',`${baseline}:${path}`]).replace(/\r\n/g,'\n');if(path==='smart-consult/index.html')expectedHtml=expectedHtml.replace('?v=flow-v1','?v=price-v1');else if(path.endsWith('/tm.html'))expectedHtml=expectedHtml.replace(/\bAG60\b/g,'AGM60');else expectedHtml=expectedHtml.replace('AGM70, AGM60, AGM80, AG60 등','AGM70, AGM60, AGM80, AGM95 등').replace('<strong>AG60</strong>','<strong>AGM60</strong>');assert.equal(fs.readFileSync(path,'utf8').replace(/\r\n/g,'\n'),expectedHtml,path);}
...
fs.mkdirSync('docs/evidence/battery-pricing',{recursive:true});fs.writeFileSync('docs/evidence/battery-pricing/pricing.json',JSON.stringify({status:'PASS',cases,metrics,coverage,transcripts,htmlChanges},null,2)+'\n');
